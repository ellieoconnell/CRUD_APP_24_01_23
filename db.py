...
def find_orders_id(id):
    query = f"SELECT order_id FROM orders_db WHERE orders_id = {id};"
    data = run_query(query)
    return True
...
def update_by_id(first_name, drink_types, drink_size, cream, drink_amount, id):
    query = f"UPDATE orders_db SET first_name = {first_name}, drink_type = {drink_types}, drink_size = {drink_size}, cream = {cream}, drink_amount = {drink_amount} WHERE order_id = {id};"
    data = run_query(query)
    return True
